...
#[allow(dead_code)]
pub fn glob_match_with_captures(glob: &str, path: &str) -> Option<Vec<Capture, MAX_CAPTURES>> {
...
fn glob_match_internal(
    glob: &str,
    path: &str,
    mut captures: Option<&mut Vec<Capture, MAX_CAPTURES>>,
) -> bool {
...
impl State {
    ...
    fn begin_capture(
        &self,
        captures: &mut Option<&mut Vec<Capture, MAX_CAPTURES>>,
        capture: Capture,
    ) {
        if let Some(captures) = captures {
            if self.capture_index < captures.len() {
                ...
                captures.push(capture).unwrap();
            }
        }
    }

    #[inline(always)]
    fn extend_capture(&self, captures: &mut Option<&mut Vec<Capture, MAX_CAPTURES>>) {
        if let Some(captures) = captures {
            if self.capture_index < captures.len() {
    ...
    #[inline(always)]
    fn end_capture(&mut self, captures: &mut Option<&mut Vec<Capture, MAX_CAPTURES>>) {
        if let Some(captures) = captures {
            if self.capture_index < captures.len() {
    ...
    #[inline(always)]
    fn add_char_capture(&mut self, captures: &mut Option<&mut Vec<Capture, MAX_CAPTURES>>) {
                ...
                c @ (b'*' | b'?' | b'[') if !in_brackets => {
                    ...
                    if let Some(captures) = captures {
                        if capture_index < captures.len() {
                            ...
                            captures.push(self.path_index..self.path_index).unwrap();
                        }
                        capture_index += 1;
                    }
                    if c == b'*'
...
mod tests {
    ...
    #[test]
    fn captures() {
        fn test_captures<'a>(glob: &str, path: &'a str) -> Option<Vec<&'a str, MAX_CAPTURES>> {
            glob_match_with_captures(glob, path)
                .map(|v| v.into_iter().map(|capture| &path[capture]).collect())
        }

        assert_eq!(test_captures("a/b", "a/b"), Some(Vec::new()));
        assert_eq!(
            test_captures("a/*/c", "a/bx/c"),
            Some(Vec::from_slice(&["bx"]).unwrap())
        );
        assert_eq!(
            test_captures("a/*/c", "a/test/c"),
            Some(Vec::from_slice(&["test"]).unwrap())
        );
        assert_eq!(
            test_captures("a/*/c/*/e", "a/b/c/d/e"),
            Some(Vec::from_slice(&["b", "d"]).unwrap())
        );
        assert_eq!(
            test_captures("a/*/c/*/e", "a/b/c/d/e"),
            Some(Vec::from_slice(&["b", "d"]).unwrap())
        ...
        assert_eq!(
            test_captures("a/{b,x}/c", "a/b/c"),
            Some(Vec::from_slice(&["b"]).unwrap())
        );
        assert_eq!(
            test_captures("a/{b,x}/c", "a/x/c"),
            Some(Vec::from_slice(&["x"]).unwrap())
        );
        assert_eq!(
            test_captures("a/?/c", "a/b/c"),
            Some(Vec::from_slice(&["b"]).unwrap())
        );
        assert_eq!(
            test_captures("a/*?x/c", "a/yybx/c"),
            Some(Vec::from_slice(&["yy", "b"]).unwrap())
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            Some(Vec::from_slice(&["bdc", "d"]).unwrap())
        );
        assert_eq!(
            test_captures("a/{b*,c}y", "a/bdy"),
            Some(Vec::from_slice(&["bd", "d"]).unwrap())
        );
        assert_eq!(
            test_captures("a/{b*c,c}", "a/bdc"),
            Some(Vec::from_slice(&["bdc", "d"]).unwrap())
        );
        assert_eq!(
            test_captures("a/{b*,c}", "a/bd"),
            Some(Vec::from_slice(&["bd", "d"]).unwrap())
        );
        assert_eq!(
            test_captures("a/{b*,c}", "a/c"),
            Some(Vec::from_slice(&["c", ""]).unwrap())
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            Some(Vec::from_slice(&["cd", "", "d"]).unwrap())
        );
        assert_eq!(
            test_captures("a/{b,c}", "a/b"),
            Some(Vec::from_slice(&["b"]).unwrap())
        );
        assert_eq!(
            test_captures("a/{b,c}", "a/c"),
        ...
        assert_eq!(
            ...
            Some(Vec::from_slice(&["c}xx", "}", "xx"]).unwrap())
        );
        ...
        assert_eq!(
            test_captures("test/*", "test/foo"),
            Some(Vec::from_slice(&["foo"]).unwrap())
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            Some(Vec::from_slice(&["foo", "baz"]).unwrap())
        );
        assert_eq!(
            test_captures("test/*.js", "test/foo.js"),
            Some(Vec::from_slice(&["foo"]).unwrap())
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            Some(Vec::from_slice(&["", "a"]).unwrap())
        );
        assert_eq!(
            test_captures("test/**/*.js", "test/dir/a.js"),
            Some(Vec::from_slice(&["dir", "a"]).unwrap())
        );
        assert_eq!(
            test_captures("test/**/*.js", "test/dir/test/a.js"),
            Some(Vec::from_slice(&["dir/test", "a"]).unwrap())
        );
        assert_eq!(
            test_captures("**/*.js", "test/dir/a.js"),
            Some(Vec::from_slice(&["test/dir", "a"]).unwrap())
        );
        assert_eq!(
            test_captures("**/**/**/**/a", "foo/bar/baz/a"),
            Some(Vec::from_slice(&["foo/bar/baz"]).unwrap())
        );
        assert_eq!(
            test_captures("a/{b/**/y,c/**/d}", "a/b/y"),
            Some(Vec::from_slice(&["b/y", "", ""]).unwrap())
        );
        assert_eq!(
            test_captures("a/{b/**/y,c/**/d}", "a/b/x/x/y"),
            Some(Vec::from_slice(&["b/x/x/y", "x/x", ""]).unwrap())
        );
        assert_eq!(
            test_captures("a/{b/**/y,c/**/d}", "a/c/x/x/d"),
            Some(Vec::from_slice(&["c/x/x/d", "", "x/x"]).unwrap())
        );
        assert_eq!(
            test_captures("a/{b/**/**/y,c/**/**/d}", "a/b/x/x/x/x/x/y"),
            Some(Vec::from_slice(&["b/x/x/x/x/x/y", "x/x/x/x/x", ""]).unwrap())
        );
        assert_eq!(
            test_captures("a/{b/**/**/y,c/**/**/d}", "a/c/x/x/x/x/x/d"),
            Some(Vec::from_slice(&["c/x/x/x/x/x/d", "", "x/x/x/x/x"]).unwrap())
        );
        assert_eq!(
            test_captures(
                "some/**/{a,b,c}/**/needle.txt",
            ...
            Some(Vec::from_slice(&["path", "a", "to/the"]).unwrap())
        );
    }
